...
export async function listScheduleCancellationCandidates(input: {
  organizations: unknown;
  scheduledDates?: unknown;
}) {
  const requestedOrganizations = Array.isArray(input.organizations)
    ? input.organizations.map((value) => clean(value).slice(0, 120)).filter(Boolean).slice(0, 20)
    : [];
  const requestedDates = new Set(
    Array.isArray(input.scheduledDates)
      ? input.scheduledDates.map(validDate).filter(Boolean).slice(0, 20)
      : [],
  );
  if (!requestedOrganizations.length) return [] as OrganizationSchedule[];
  ...
  const rows = result.results.map(scheduleJson);
  const existingNames = [...new Set(rows.map((row) => row.organization).filter(Boolean))];
  const matchedKeys = new Set<string>();
  requestedOrganizations.forEach((requested) => {
    const exactKey = institutionAliasKey(requested);
    if (existingNames.some((name) => institutionAliasKey(name) === exactKey)) {
      matchedKeys.add(exactKey);
      return;
    }
    const similar = findSimilarInstitutionNames(requested, existingNames, 2);
    if (similar.length === 1) matchedKeys.add(institutionAliasKey(similar[0]));
  });

  const today = new Date(Date.now() + 9 * 60 * 60 * 1_000).toISOString().slice(0, 10);
  const fallbackDate = new Date(`${today}T00:00:00Z`);
  fallbackDate.setUTCDate(fallbackDate.getUTCDate() + 31);
  const fallbackEnd = fallbackDate.toISOString().slice(0, 10);
  return rows.filter((row) => {
    if (!matchedKeys.has(institutionAliasKey(row.organization))) return false;
    if (requestedDates.size) return requestedDates.has(row.scheduledDate);
    return row.scheduledDate >= today && row.scheduledDate <= fallbackEnd;
  });
}
