...
function settlementSheet(input: ConsortiumSettlementWorkbookInput, hasDrawing: boolean) {
  ...
  const summaryRow = lastCostRow + 2;
  const signRow = summaryRow + 7;
  ...
  const finalPayment = Math.max(0, gross - consortiumCost);
  ...
  const itemRows = input.items.length ? input.items.map((item, index) => {
    ...
    return `<row r="${row}" ht="31" customHeight="1">${numeric(`A${row}`, index + 1, 7)}${inline(`B${row}`, item.name, 8)}${inline(`C${row}`, item.contractLabel, 7)}${numeric(`D${row}`, item.lineAmount, 9)}${rate(`E${row}`, item.consortiumRate, 10)}${formula(`F${row}`, `FLOOR(D${row}*E${row},10)`, item.grossPayment, 9)}${inline(`G${row}`, "", 8)}</row>`;
  ...
  const costRows = input.costs.length ? input.costs.map((cost, index) => {
    ...
    return `<row r="${row}" ht="29" customHeight="1">${numeric(`A${row}`, index + 1, 7)}${inline(`B${row}`, cost.label, 8)}${numeric(`C${row}`, cost.amount, 9)}${inline(`D${row}`, treatment, 7)}${formula(`E${row}`, `IF(D${row}="정산서 반영",C${row},0)`, cost.consortiumDeduction, 11)}${inline(`F${row}`, cost.bearer === "consortium" ? "지급 예정액에 반영" : "정산서 금액 미반영", 8)}${inline(`G${row}`, "", 8)}</row>`;
  }).join("") : `<row r="${firstCostRow}" ht="29" customHeight="1">${inline(`A${firstCostRow}`, "", 7)}${inline(`B${firstCostRow}`, "별도 비용 없음", 8)}${numeric(`C${firstCostRow}`, 0, 9)}${inline(`D${firstCostRow}`, "-", 7)}${numeric(`E${firstCostRow}`, 0, 11)}${inline(`F${firstCostRow}`, "", 8)}${inline(`G${firstCostRow}`, "", 8)}</row>`;
  ...
  const merges = [
    ...
    ...Array.from({ length: costCount }, (_, index) => `F${firstCostRow + index}:G${firstCostRow + index}`),
    ...
    `A${signRow}:C${signRow + 2}`, `D${signRow}:F${signRow + 2}`, `G${signRow}:G${signRow + 2}`,
  ];

  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><dimension ref="A1:G${signRow + 2}"/><sheetViews><sheetView showGridLines="0" zoomScale="90" workbookViewId="0"/></sheetViews><sheetFormatPr defaultRowHeight="20"/><cols><col min="1" max="1" width="11" customWidth="1"/><col min="2" max="2" width="21" customWidth="1"/><col min="3" max="3" width="14" customWidth="1"/><col min="4" max="4" width="18" customWidth="1"/><col min="5" max="5" width="18" customWidth="1"/><col min="6" max="6" width="16" customWidth="1"/><col min="7" max="7" width="16" customWidth="1"/></cols><sheetData>
...
<row r="12" ht="40" customHeight="1">${inline("A12", "최종 지급 예정액 (VAT 포함)", 16)}${inline("B12", "", 16)}${inline("C12", koreanAmount(finalPayment), 16)}${styledBlanks(12, ["D", "E"], 16)}${formula("F12", `MAX(0,SUM(F${firstItemRow}:F${lastItemRow})-SUM(E${firstCostRow}:E${lastCostRow}))`, finalPayment, 17)}${inline("G12", "", 17)}</row>
...
<row r="${firstCostRow - 1}" ht="27" customHeight="1">${inline(`A${firstCostRow - 1}`, "No", 6)}${inline(`B${firstCostRow - 1}`, "비용 항목", 6)}${inline(`C${firstCostRow - 1}`, "금액", 6)}${inline(`D${firstCostRow - 1}`, "비용 처리 방식", 6)}${inline(`E${firstCostRow - 1}`, "정산 반영액", 6)}${inline(`F${firstCostRow - 1}`, "반영 결과", 6)}${inline(`G${firstCostRow - 1}`, "", 6)}</row>${costRows}
...
<row r="${summaryRow + 2}" ht="23" customHeight="1">${inline(`A${summaryRow + 2}`, "비용 처리", 3)}${inline(`B${summaryRow + 2}`, "정산서 반영 항목 포함", 4)}${styledBlanks(summaryRow + 2, ["C", "D"], 4)}${inline(`E${summaryRow + 2}`, "정산 반영 비용", 3)}${formula(`F${summaryRow + 2}`, `SUM(E${firstCostRow}:E${lastCostRow})`, consortiumCost, 11)}${inline(`G${summaryRow + 2}`, "", 11)}</row>
...
<row r="${summaryRow + 5}" ht="31" customHeight="1">${inline(`A${summaryRow + 5}`, "안내", 3)}${inline(`B${summaryRow + 5}`, "실제 지급일은 상호 확인 후 확정합니다.", 4)}${styledBlanks(summaryRow + 5, ["C", "D"], 4)}${inline(`E${summaryRow + 5}`, "최종 지급 예정액", 16)}${formula(`F${summaryRow + 5}`, `MAX(0,F${summaryRow + 1}-F${summaryRow + 2})`, finalPayment, 17)}${inline(`G${summaryRow + 5}`, "", 17)}</row>
<row r="${signRow}" ht="28" customHeight="1">${inline(`A${signRow}`, `위와 같이 정산합니다.\n\n${koreanDate(input.quoteDate)}`, 19)}${styledBlanks(signRow, ["B", "C"], 19)}${inline(`D${signRow}`, "주식회사 위즈업\n대표이사  박 원 석", 19)}${styledBlanks(signRow, ["E", "F"], 19)}${inline(`G${signRow}`, "", 19)}</row><row r="${signRow + 1}" ht="28" customHeight="1"/><row r="${signRow + 2}" ht="28" customHeight="1"/>
...
const styles = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><numFmts count="2"><numFmt numFmtId="164" formatCode="#,##0&quot;원&quot;;[Red](#,##0)&quot;원&quot;;-"/><numFmt numFmtId="165" formatCode="0.0%"/></numFmts><fonts count="7"><font><sz val="9"/><name val="맑은 고딕"/></font><font><b/><sz val="22"/><color rgb="FF182842"/><name val="맑은 고딕"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="10"/><name val="맑은 고딕"/></font><font><b/><color rgb="FF52617D"/><sz val="9"/><name val="맑은 고딕"/></font><font><b/><color rgb="FF2254D1"/><sz val="18"/><name val="맑은 고딕"/></font><font><b/><color rgb="FFC24B3F"/><sz val="10"/><name val="맑은 고딕"/></font><font><b/><sz val="11"/><name val="맑은 고딕"/></font></fonts><fills count="7"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF182842"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFF4F7FC"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFEAF1FF"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FF3157E6"/></patternFill></fill><fill><patternFill patternType="solid"><fgColor rgb="FFFFF5E8"/></patternFill></fill></fills><borders count="3"><border><left/><right/><top/><bottom/><diagonal/></border><border><left style="thin"><color rgb="FFCCD6E6"/></left><right style="thin"><color rgb="FFCCD6E6"/></right><top style="thin"><color rgb="FFCCD6E6"/></top><bottom style="thin"><color rgb="FFCCD6E6"/></bottom><diagonal/></border><border><left/><right/><top style="medium"><color rgb="FF3157E6"/></top><bottom style="medium"><color rgb="FF3157E6"/></bottom><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="20"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/><xf numFmtId="0" fontId="1" fillId="0" borderId="0" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="2" fillId="2" borderId="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="3" fillId="3" borderId="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" applyAlignment="1"><alignment horizontal="left" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="2" fillId="2" borderId="1" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" applyAlignment="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="1" applyAlignment="1"><alignment horizontal="left" vertical="center" wrapText="1"/></xf><xf numFmtId="164" fontId="0" fillId="0" borderId="1" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="165" fontId="0" fillId="0" borderId="1" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="164" fontId="5" fillId="6" borderId="1" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="0" fontId="6" fillId="4" borderId="2" applyAlignment="1"><alignment horizontal="left" vertical="center"/></xf><xf numFmtId="164" fontId="4" fillId="4" borderId="2" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="164" fontId="5" fillId="6" borderId="2" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="0" fontId="0" fillId="5" borderId="0"/><xf numFmtId="0" fontId="6" fillId="4" borderId="2" applyAlignment="1"><alignment horizontal="left" vertical="center"/></xf><xf numFmtId="164" fontId="4" fillId="4" borderId="2" applyNumberFormat="1" applyAlignment="1"><alignment horizontal="right" vertical="center"/></xf><xf numFmtId="0" fontId="0" fillId="3" borderId="0" applyAlignment="1"><alignment horizontal="left" vertical="center" wrapText="1"/></xf><xf numFmtId="0" fontId="6" fillId="0" borderId="1" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>`;
...
export function createConsortiumSettlementWorkbook(input: ConsortiumSettlementWorkbookInput) {
  ...
  const itemCount = Math.max(1, input.items.length);
  const costCount = Math.max(1, input.costs.length);
  const signRow = (15 + itemCount - 1) + 2 + 2 + costCount - 1 + 2 + 7;
