...
function buildSheet(items: ProductCatalogItem[]) {
  const header = HEADERS.map((value, index) =>
    inlineCell(`${columnName(index)}1`, value, 4),
  ).join("");
  const rows = items
    .map((item, index) => {
      const row = index + 2;
      return `<row r="${row}">
        ...
        ${inlineCell(`E${row}`, item.supplyType === "direct" ? "위즈업 직접 공급" : "협력사 공급")}
...
export function createProductCatalogWorkbook(items: ProductCatalogItem[]) {
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF354BFF"/><bgColor indexed="64"/></patternFill></fill></fills>
...
<cp:coreProperties xmlns:cp="http://schemas.openxmlformats.org/package/2006/metadata/core-properties" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dcterms="http://purl.org/dc/terms/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
  <dc:title>위즈업 제품 기준 정보</dc:title><dc:creator>WHIZZUP</dc:creator><cp:lastModifiedBy>WHIZZUP</cp:lastModifiedBy>
  <dcterms:created xsi:type="dcterms:W3CDTF">${new Date().toISOString()}</dcterms:created>
</cp:coreProperties>`),
    "docProps/app.xml": strToU8(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties" xmlns:vt="http://schemas.openxmlformats.org/officeDocument/2006/docPropsVTypes">
  <Application>WHIZZUP Sales Hub</Application>
</Properties>`),
  };
...
function parseNullableNumber(value: string, kind: "price" | "rate") {
  ...
  if (!Number.isFinite(parsed) || parsed < 0) {
    return { value: null, error: "숫자 형식을 확인해 주세요." };
  }
  if (kind === "rate") {
    const normalized = value.includes("%") || parsed > 1 ? parsed / 100 : parsed;
    if (normalized > 1) {
      return { value: null, error: "비율은 100% 이하로 입력해 주세요." };
    }
...
export function parseProductCatalogWorkbook(buffer: ArrayBuffer) {
  ...
  const headerMap = new Map(
    Object.entries(headerRow.values).map(([column, value]) => [
      value.trim(),
      column,
    ]),
  );
  ...
  return parsedRows
    ...
    .map(({ rowNumber, values }): ProductCatalogImportRow | null => {
      ...
      const unitPriceResult = parseNullableNumber(
        values[columnFor("단가")] ?? "",
        "price",
      );
      const rateResult = parseNullableNumber(values[rateColumn] ?? "", "rate");
      const supplyTypeText = hasSupplyTypeColumn
        ? (values[columnFor("공급 구분")] ?? "").trim()
        : "";
      const supplyType: ProductSupplyType | null = !hasSupplyTypeColumn
        ? null
        : /직접/.test(supplyTypeText)
          ? "direct"
      ...
      if (rateResult.error) {
        errors.push(`수수료율 / 마진율: ${rateResult.error}`);
      }
      if (hasSupplyTypeColumn && supplyType === null) {
        errors.push("공급 구분: ‘협력사 공급’ 또는 ‘위즈업 직접 공급’으로 입력해 주세요.");
      }
      return {
        rowNumber,
        name,
        ...
        supplyType,
        commissionRate:
          supplyType === "direct" ? null : rateResult.value,
        marginRate:
          supplyType === "direct" ? rateResult.value : null,
        reference,
        errors,
      };
    })
