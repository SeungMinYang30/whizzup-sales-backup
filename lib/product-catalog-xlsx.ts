  ...
  const rows = items
    .map((item, index) => {
      ...
      return `<row r="${row}">
        ...
        ${numberCell(`E${row}`, item.commissionRate, 3)}
        ${inlineCell(`F${row}`, item.reference)}
      </row>`;
    })
    .join("");
...
function parseNullableNumber(value: string, kind: "price" | "rate") {
  ...
  if (kind === "rate") {
    ...
    if (normalized > 1) {
      return { value: null, error: "수수료율은 100% 이하로 입력해 주세요." };
    }
...
export function parseProductCatalogWorkbook(buffer: ArrayBuffer) {
  ...
  return parsedRows
    ...
    .map(({ rowNumber, values }): ProductCatalogImportRow | null => {
      ...
      const commissionResult = parseNullableNumber(
        values[columnFor("수수료율")] ?? "",
        "rate",
      );
      ...
      if (commissionResult.error) {
        errors.push(`수수료율: ${commissionResult.error}`);
      }
      return {
        rowNumber,
        name,
