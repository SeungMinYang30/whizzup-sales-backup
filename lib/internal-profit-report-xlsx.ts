...
function sheetXml(input: InternalProfitReportWorkbookInput, hasDrawing: boolean) {
  ...
  const hasCostDetails = costDetails.length > 0;
  const firstRow = 13;
  const lastRow = firstRow + Math.max(1, input.rows.length) - 1;
  const costHeaderRow = lastRow + 2;
  const costFirstRow = costHeaderRow + 1;
  const costLastRow = costFirstRow + costDetails.length - 1;
  const summaryRow = hasCostDetails ? costLastRow + 2 : lastRow + 2;
  ...
  const costRows = costDetails.map((detail, index) => {
    const row = costFirstRow + index;
    const displayLabel = detail.itemName && detail.itemName !== detail.label
      ? `${detail.label} · ${detail.itemName}`
      : detail.label;
  ...
  const costMerges = costDetails.flatMap((_,index)=>{
    const row = costFirstRow + index;
    return [`A${row}:D${row}`,`E${row}:I${row}`,`J${row}:L${row}`];
  });
  const costSectionMerges = hasCostDetails ? [`A${costHeaderRow}:L${costHeaderRow}`,...costMerges] : [];
  const merges = ["C2:J3","A5:B5","C5:F5","G5:H5","I5:L5","A6:B6","C6:F6","G6:H6","I6:L6","A7:B7","C7:F7","G7:H7","I7:L7",...mergePairs,"A12:L12",...itemMerges,...costSectionMerges,`A${summaryRow}:H${summaryRow}`,`I${summaryRow}:L${summaryRow}`,`A${noteRow}:L${noteRow}`];
...
${hasCostDetails ? `<row r="${costHeaderRow}" ht="24" customHeight="1">${textCell(`A${costHeaderRow}`,"내부 비용·지원·콘텐츠 대체 상세",2)}${blanks(costHeaderRow,["B","C","D","E","F","G","H","I","J","K","L"],2)}</row>${costRows}` : ""}
