...
function sheetXml(input: InternalProfitReportWorkbookInput, hasDrawing: boolean) {
  ...
  const merges = ["C2:J3","A5:B5","C5:F5","G5:H5","I5:L5","A6:B6","C6:F6","G6:H6","I6:L6","A7:B7","C7:F7","G7:H7","I7:L7",...mergePairs,"A12:L12",...itemMerges,`A${summaryRow}:J${summaryRow}`,`K${summaryRow}:L${summaryRow}`,`A${noteRow}:L${noteRow}`];
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><dimension ref="A1:R${endRow}"/><sheetViews><sheetView showGridLines="0" zoomScale="90" workbookViewId="0"/></sheetViews><sheetFormatPr defaultRowHeight="20"/><cols><col min="1" max="1" width="5" customWidth="1"/><col min="2" max="2" width="11" customWidth="1"/><col min="3" max="12" width="8.5" customWidth="1"/><col min="13" max="18" width="2" hidden="1" customWidth="1"/></cols><sheetData>
...
<row r="${summaryRow}" ht="31" customHeight="1">${textCell(`A${summaryRow}`,"컨소·내부 비용을 반영한 최종 예상 수익",6)}${blanks(summaryRow,["B","C","D","E","F","G","H","I","J"],6)}${formulaCell(`K${summaryRow}`,"I10",input.margin,17)}${textCell(`L${summaryRow}`,"",17)}</row>
