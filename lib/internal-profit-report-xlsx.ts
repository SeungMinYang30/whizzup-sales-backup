...
function sheetXml(input: InternalProfitReportWorkbookInput) {
  if (input.compactView) return compactSheetXml(input);
  const firstRow = 15;
  ...
  const itemRows = input.rows.length ? input.rows.map((item, index) => {
    ...
    const height = label.length > 54 ? 58 : label.length > 30 ? 50 : 44;
    return `<row r="${row}" ht="${height}" customHeight="1">${numberCell(`A${row}`, item.number, 8)}${textCell(`B${row}`, label, 9)}${numberCell(`C${row}`, item.quantity, 8)}${textCell(`D${row}`, item.unit, 8)}${numberCell(`E${row}`, item.unitPrice, 10)}${item.complimentary ? numberCell(`F${row}`, 0, 10) : formulaCell(`F${row}`, `C${row}*E${row}`, item.amount, 10)}${rateCell(`G${row}`, item.baseRate, 11)}${formulaCell(`H${row}`, `FLOOR(F${row}*G${row},10)`, item.baseEarning, 10)}${numberCell(`I${row}`, item.earning, 10)}${rateCell(`J${row}`, item.consortiumRate, 11)}${numberCell(`K${row}`, item.consortium, 12)}${numberCell(`L${row}`, item.internalCostDisplay, 12)}${formulaCell(`M${row}`, `I${row}-K${row}-L${row}`, item.netProfit, 13)}${textCell(`N${row}`, item.complimentary ? "무상 제공" : item.status, 8)}</row>`;
  ...
  const merges = ["A2:N3", "A5:B5", "C5:G5", "H5:I5", "J5:N5", "A6:B6", "C6:G6", "H6:I6", "J6:N6", "A7:B7", "C7:G7", "H7:I7", "J7:N7",
    "A9:B9", "A10:B10", "C9:D9", "C10:D10", "E9:F9", "E10:F10", "G9:H9", "G10:H10", "I9:J9", "I10:J10", "K9:N9", "K10:N10", "A12:N12",
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><dimension ref="A1:N${endRow}"/><sheetViews><sheetView showGridLines="0" zoomScale="100" workbookViewId="0"><pane ySplit="14" topLeftCell="A15" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="21"/><cols><col min="1" max="1" width="5" customWidth="1"/><col min="2" max="2" width="28" customWidth="1"/><col min="3" max="4" width="11" customWidth="1"/><col min="5" max="6" width="14" customWidth="1"/><col min="7" max="7" width="8" customWidth="1"/><col min="8" max="9" width="14" customWidth="1"/><col min="10" max="10" width="8" customWidth="1"/><col min="11" max="12" width="14" customWidth="1"/><col min="13" max="13" width="18" customWidth="1"/><col min="14" max="14" width="12" customWidth="1"/></cols><sheetData>
...
<row r="14" ht="38" customHeight="1">${["No","품목·규격","수량","단위","단가","견적금액","기준률","기준 수익","반영 수익","컨소율","컨소 지급","대체·내부비용","품목 순이익","상태"].map((v,i)=>textCell(`${String.fromCharCode(65+i)}14`,v,7)).join("")}</row>${itemRows}
...
function compactSheetXml(input: InternalProfitReportWorkbookInput) {
  const firstRow = 16;
  const lastRow = firstRow + Math.max(1, input.rows.length) - 1;
  const summaryRow = lastRow + 2;
  const endRow = summaryRow + 2;
  const itemRows = input.rows.length ? input.rows.map((item, index) => {
    const row = firstRow + index;
    const label = item.specification ? `${item.name}\n${item.specification}` : item.name;
    const cost = item.consortium + item.internalCostDisplay;
    const height = label.length > 48 ? 62 : 52;
    return `<row r="${row}" ht="${height}" customHeight="1">${numberCell(`A${row}`,item.number,8)}${textCell(`B${row}`,label,9)}${numberCell(`C${row}`,item.amount,10)}${numberCell(`D${row}`,item.earning,10)}${numberCell(`E${row}`,cost,12)}${formulaCell(`F${row}`,`D${row}-E${row}`,item.netProfit,13)}${textCell(`G${row}`,item.complimentary ? "무상 제공" : item.status,8)}</row>`;
  }).join("") : `<row r="${firstRow}" ht="42" customHeight="1">${textCell(`A${firstRow}`,"",8)}${textCell(`B${firstRow}`,"품목이 없습니다.",9)}${blanks(firstRow,["C","D","E","F","G"],8)}</row>`;
  const merges = ["A2:G3","A5:B5","C5:G5","A6:B6","C6:G6","A7:B7","C7:G7","A9:B9","C9:D9","E9:G9","A10:B10","C10:D10","E10:G10","A12:G12",`A${summaryRow}:D${summaryRow}`,`E${summaryRow}:G${summaryRow}`,`A${summaryRow+1}:D${summaryRow+1}`,`E${summaryRow+1}:G${summaryRow+1}`];
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><sheetPr><pageSetUpPr fitToPage="1"/></sheetPr><dimension ref="A1:G${endRow}"/><sheetViews><sheetView showGridLines="0" zoomScale="100" workbookViewId="0"><pane ySplit="15" topLeftCell="A16" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><sheetFormatPr defaultRowHeight="22"/><cols><col min="1" max="1" width="6" customWidth="1"/><col min="2" max="2" width="34" customWidth="1"/><col min="3" max="6" width="17" customWidth="1"/><col min="7" max="7" width="15" customWidth="1"/></cols><sheetData>
<row r="1" ht="5" customHeight="1">${textCell("A1","",14)}${blanks(1,["B","C","D","E","F","G"],14)}</row>
<row r="2" ht="28" customHeight="1">${textCell("A2","내 부  수 익 표",1)}</row><row r="3" ht="28" customHeight="1"/>
<row r="5" ht="24" customHeight="1">${textCell("A5","기관",3)}${textCell("C5",input.organization,4)}</row>
<row r="6" ht="24" customHeight="1">${textCell("A6","사업·견적명",3)}${textCell("C6",input.projectTitle || "미입력",4)}</row>
<row r="7" ht="24" customHeight="1">${textCell("A7","견적번호",3)}${textCell("C7",`${input.quoteNumber || "저장 전"} · ${input.quoteDate}`,4)}</row>
<row r="9" ht="24" customHeight="1">${textCell("A9","견적금액",5)}${textCell("C9","예상 수익",5)}${textCell("E9","최종 총이익 · 마진율",5)}</row>
<row r="10" ht="36" customHeight="1">${numberCell("A10",input.total,15)}${numberCell("C10",input.earning,15)}${textCell("E10",`${new Intl.NumberFormat("ko-KR").format(input.margin)}원 · ${(input.marginRate*100).toFixed(1)}%`,4)}</row>
<row r="12" ht="28" customHeight="1">${textCell("A12","품목별 수익 내역 · 모바일 간편 보기",2)}</row>
<row r="15" ht="38" customHeight="1">${["No","품목·규격","견적금액","예상 수익","컨소·내부비용","품목 순이익","상태"].map((v,i)=>textCell(`${String.fromCharCode(65+i)}15`,v,7)).join("")}</row>${itemRows}
<row r="${summaryRow}" ht="28" customHeight="1">${textCell(`A${summaryRow}`,"최종 총이익",2)}${numberCell(`E${summaryRow}`,input.margin,17)}</row>
<row r="${summaryRow+1}" ht="28" customHeight="1">${textCell(`A${summaryRow+1}`,"마진율",6)}${rateCell(`E${summaryRow+1}`,input.marginRate,18)}</row>
</sheetData><mergeCells count="${merges.length}">${merges.map((ref)=>`<mergeCell ref="${ref}"/>`).join("")}</mergeCells><printOptions horizontalCentered="1"/><pageMargins left="0.2" right="0.2" top="0.3" bottom="0.3" header="0.15" footer="0.15"/><pageSetup paperSize="9" orientation="landscape" fitToWidth="1" fitToHeight="0" horizontalDpi="300" verticalDpi="300"/></worksheet>`;
}
...
export function createInternalProfitReportWorkbook(input: InternalProfitReportWorkbookInput) {
  ...
  return zipSync({
    ...
    "xl/workbook.xml": bytes(`<?xml version="1.0" encoding="UTF-8" standalone="yes"?><workbook xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main" xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"><sheets><sheet name="내부 수익표" sheetId="1" r:id="rId1"/></sheets><definedNames><definedName name="_xlnm.Print_Area" localSheetId="0">'내부 수익표'!$A$1:$${lastColumn}$${endRow}</definedName></definedNames><calcPr calcId="191029" calcMode="auto" fullCalcOnLoad="1" forceFullCalc="1"/></workbook>`),
