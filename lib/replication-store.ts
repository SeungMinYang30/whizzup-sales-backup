...
let replicationStateTableReady: Promise<void> | null = null;

function ensureReplicationStateTable() {
  if (!replicationStateTableReady) {
    replicationStateTableReady = getD1()
      .prepare(
        `CREATE TABLE IF NOT EXISTS replication_sync_state (
           ...
           operating_mode TEXT NOT NULL DEFAULT 'replica'
             CHECK (operating_mode IN ('replica', 'primary')),
           cutover_at TEXT,
           cutover_by INTEGER
         )`,
      )
      ...
      .then(() => undefined)
      .catch((error) => {
        replicationStateTableReady = null;
        throw error;
      });
  }
  return replicationStateTableReady;
}
...
export async function getReplicationSyncState() {
  await ensureReplicationStateTable();
  return getD1()
    .prepare(
