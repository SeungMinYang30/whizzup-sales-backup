...
test("원본 우선순위와 제목 비추론 원칙을 지킨다", () => {
  ...
  assert.match(calendar, /해당 기관에서 직접 입력한 공정만 다음 선택부터 재사용됩니다/);
...
test("Google API 등록·수정·삭제는 사이트 일정 ID로 중복을 방지한다", () => {
  ...
  assert.match(route, /mergeCalendarSchedules\(/);
  assert.match(route, /listReadOnlyGoogleCalendarRange\(start, end\)/);
  assert.match(route, /googleCalendarWritable: false/);
...
test("시공 일정표 기관 삭제는 본계정만 노출·실행하고 모바일은 현황 필터를 제공한다", () => {
  assert.match(route, /시공·납품 일정표의 기관 숨김·다시 표시는 기본 운영자만/);
  assert.match(route, /board\.projects\.filter\(\(project\) => !project\.hidden\)/);
...
test("schedule category changes reuse the existing Google event and clean only matched orphans", () => {
  ...
  assert.match(route, /const schedule = await updateOrganizationSchedule\(/);
  assert.match(route, /queueGoogleCalendarSync\(\{ ids: \[schedule\.id\]/);
