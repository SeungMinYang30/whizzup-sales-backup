...
test("sales summary counts the latest actual activity per canonical institution", () => {
  assert.deepEqual(summarizeSalesDashboard(rows), {
    total: 2,
    active: 1,
    completed: 1,
  });
...
test("sales summary is independent from API row arrival order", () => {
  assert.deepEqual(
    summarizeSalesDashboard(rows),
    summarizeSalesDashboard([...rows].reverse()),
  );
});
...
test("system-only registration and award-import rows are excluded", () => {
  const systemRows = [
    ...
    },
    {
      ...
      activity_date: "2026-08-15",
      source_chat: "수주업체 관리",
      activity_type: "협력사 등록",
    },
  ];
  assert.deepEqual(summarizeSalesDashboard([...rows, ...systemRows]), {
    total: 2,
    active: 1,
    completed: 1,
  });
