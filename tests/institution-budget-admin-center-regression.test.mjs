...
test("budget manager initializes the latest D1 schema and uses PostgreSQL-safe detail ordering", async () => {
  ...
  assert.match(
    budgets,
    /ORDER BY COALESCE\([\s\S]{0,80}a\.activity_date[\s\S]{0,120}CAST\(p\.updated_at AS TEXT\)[\s\S]{0,120}CAST\(p\.created_at AS TEXT\)[\s\S]{0,40}\) DESC/,
  );
...
test("budget audit and review exclusion tables exist in the D1 schema and backup flow", async () => {
...
test("unclassified review exclusions are entity-scoped and never rewrite originals", async () => {
  ...
  assert.match(budgets, /CAST\(p\.updated_at AS TEXT\)/);
  assert.match(budgets, /CAST\(p\.created_at AS TEXT\)/);
