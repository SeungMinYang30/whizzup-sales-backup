...
const readability = fs.readFileSync(path.join(root, "app", "readability.css"), "utf8");
...
test("dashboard activity scope uses two equal centered mobile columns", () => {
  ...
  assert.match(
    readability,
    /\.dashboard-records \.records-heading-actions\s*\{[^}]*grid-template-columns:\s*minmax\(0,\s*1fr\)\s*!important;[^}]*justify-items:\s*center;[^}]*\}/s,
  );
  assert.match(
    readability,
    /\.dashboard-records \.records-heading-actions > \.dashboard-activity-scope\s*\{[^}]*width:\s*176px;[^}]*max-width:\s*100%;[^}]*justify-self:\s*center;[^}]*\}/s,
  );
});
