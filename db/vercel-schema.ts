export const VERCEL_SCHEMA_VERSION = "202607220005_full_parity";
...
CREATE TABLE IF NOT EXISTS public.quotation_file_objects (
...
CREATE INDEX IF NOT EXISTS quotation_documents_organization_idx
  ON public.quotation_documents (organization, created_at);
CREATE INDEX IF NOT EXISTS deletion_batches_active_idx
  ON public.deletion_batches (restored_at, expires_at, deleted_at);
...
ALTER TABLE public.quotation_file_objects ENABLE ROW LEVEL SECURITY;
...
REVOKE ALL ON public.holdem_weekly_scores
  FROM anon, authenticated;
REVOKE ALL ON public.quotation_documents
  FROM anon, authenticated;
REVOKE ALL ON public.quotation_file_objects
  FROM anon, authenticated;
REVOKE ALL ON public.deletion_batches
  FROM anon, authenticated;
