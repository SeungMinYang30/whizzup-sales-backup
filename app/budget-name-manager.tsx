          ...
          >
            ...
            {unclassifiedAction === "connect" ? (
              <div className="budget-unclassified-modal-form">
                <label>
                  <span>표준 예산명 검색</span>
                  <input autoFocus value={targetGroupSearch} onChange={(event) => setTargetGroupSearch(event.target.value)} placeholder="표준명·별칭 검색" />
                </label>
                <label>
                  <span>연결할 표준 예산명</span>
                  <select value={targetGroupId ?? ""} onChange={(event) => setTargetGroupId(positiveInteger(event.target.value) || null)}>
                    <option value="">표준 예산명을 선택해 주세요</option>
                    {filteredActiveGroups.map((group) => <option key={group.id} value={group.id}>{group.canonicalName}</option>)}
                  </select>
                </label>
              </div>
            ) : (
              <div className="budget-unclassified-modal-form">
                <label><span>새 표준 예산명</span><input autoFocus value={newName} onChange={(event) => setNewName(event.target.value)} placeholder="선택한 이름을 표준명으로 사용" /></label>
                <label><span>예산 구분</span><select value={newKind} onChange={(event) => updateNewKind(normalizeKind(event.target.value) || "purpose")}><option value="purpose">목적예산</option><option value="self">자체예산</option></select></label>
                <label><span>금액 처리</span><select value={newAmountMode} onChange={(event) => setNewAmountMode(normalizeMode(event.target.value) || "manual")}><option value="manual">금액 직접 입력</option><option value="quote_auto">품목·견적 합계 자동 계산</option></select></label>
                <label><span>기본 예산액 (선택)</span><input inputMode="numeric" value={newDefaultAmount} onChange={(event) => setNewDefaultAmount(moneyInput(event.target.value))} placeholder="예: 50,000,000" /></label>
              </div>
            )}
            <div className="institution-confirmation-actions">
              <button type="button" onClick={() => setUnclassifiedAction(null)}>취소</button>
              <button type="button" className="primary" disabled={saving || (unclassifiedAction === "connect" ? !targetGroupId : !newName.trim())} onClick={() => void (unclassifiedAction === "connect" ? connectSelected() : registerSelected())}>
                {saving ? "처리 중…" : unclassifiedAction === "connect" ? "선택 표준명에 연결" : "새 표준명 등록"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="panel budget-name-source-panel">
          ...
          {selectedNames.length > 0 && (
            <div className="budget-unclassified-selection-actions" role="status">
              ...
              <div>
                <button type="button" onClick={() => setUnclassifiedAction("connect")}>기존 표준명에 연결</button>
                <button type="button" onClick={() => { if (!newName.trim()) setNewName(selectedNames[0] ?? ""); setUnclassifiedAction("register"); }}>새 표준명으로 등록</button>
                <button type="button" disabled={saving} onClick={() => void changeReviewExclusions(data.names.filter((item) => selectedNames.includes(item.name)).flatMap((item) => item.details), true)}>검토 목록에서 제외</button>
              </div>
            </div>
          )}
