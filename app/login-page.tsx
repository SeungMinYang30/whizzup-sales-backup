    ...
    <main className="direct-login-page">
      <section className="direct-login-card">
        ...
        <form onSubmit={submit} className="direct-login-form">
          ...
          <button className="direct-login-submit" type="submit" disabled={busy || !clientReady}>
            {!clientReady ? "로그인 준비 중" : busy ? "처리 중" : mode === "login" ? "로그인" : mode === "signup" ? "가입 요청" : "재설정 요청"}
          </button>
        </form>
        <div className="direct-login-links">
          {mode !== "login" && <button type="button" disabled={!clientReady} onClick={() => changeMode("login")}>로그인</button>}
          {mode !== "signup" && <button type="button" disabled={!clientReady} onClick={() => changeMode("signup")}>신규 가입</button>}
          {mode !== "reset" && <button type="button" disabled={!clientReady} onClick={() => changeMode("reset")}>비밀번호 재설정</button>}
        </div>
      </section>
    </main>
  );
