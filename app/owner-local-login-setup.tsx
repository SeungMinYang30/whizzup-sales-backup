...
export default function OwnerLocalLoginSetup({ email }: { email: string }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(payload.error || "대표 로그인을 설정하지 못했습니다.");
      setPassword("");
      setConfirmation("");
      setMessage(`${payload.username || email} 아이디 로그인을 설정했습니다.`);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "대표 로그인을 설정하지 못했습니다.");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="owner-local-login-setup">
      <div>
        <strong>대표 아이디 로그인</strong>
        <span>현재 대표 계정에 비밀번호 로그인을 연결합니다. Google 로그인도 계속 사용할 수 있습니다.</span>
      </div>
      {!open ? (
        <button type="button" onClick={() => setOpen(true)}>로그인 설정</button>
      ) : (
        <form onSubmit={submit}>
          <label><span>로그인 아이디</span><input value={email} readOnly /></label>
