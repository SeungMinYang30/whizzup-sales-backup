...
function mergeDetectedShowroomSchedules(existing: unknown, detected: unknown) {
  const current = parseProgressScheduleEntries(serializeProgressSchedule(existing));
  const detectedItems = parseProgressScheduleEntries(serializeProgressSchedule(detected));
  detectedItems.forEach((item) => {
    const existingIndex = current.findIndex(
      (candidate) =>
        candidate.date === item.date && isShowroomScheduleLabel(candidate.label),
    );
    if (existingIndex < 0) {
      current.push(item);
      return;
  ...
  return serializeProgressSchedule(current);
}
...
시연·데모·체험 일정은 위즈업 또는 에어패스 일정으로 확인되고 날짜가 명확할 때 progressSchedule에 넣으세요. label에는 "에어패스 시연"처럼 대상과 시연 성격이 드러나는 짧은 제목을 사용하세요.
...
확실하지 않으면 사용자가 말한 원문을 그대로 보존하세요. 최종 표준 연결은 시스템이 다시 검증합니다.`,
    ...
    const normalizedDrafts = normalizeDrafts(parsed.drafts);
    const detectedShowroomSchedules = normalizedDrafts.length === 1
      ? extractShowroomSchedulesFromText(userProjectText, postedDate)
      : [];
    const drafts: Record<string, unknown>[] = normalizedDrafts.map((draft): Record<string, unknown> => {
      ...
      const allowedProgressSchedule = serializeProgressSchedule(
        parseProgressScheduleEntries(
          mergeDetectedShowroomSchedules(
            draft.progressSchedule,
            detectedShowroomSchedules,
          ),
        ).filter(
