...
const pdfCampaignSchema = {
  ...
  required: ["campaignName", "selectionDate", "notes", "rows"],
  additionalProperties: false,
};
...
selectionDate에는 문서에서 확인되는 공고일·선정일·명단 발표일을 YYYY-MM-DD로 적고, 확인되지 않으면 빈 문자열로 두세요.
...
같은 기관이 여러 페이지에 반복되면 하나로 합치고 서로 다른 기관은 합치지 마세요.`,
    ...
    return Response.json({
      ...
      selectionDate: clean(parsed.selectionDate).slice(0, 10),
