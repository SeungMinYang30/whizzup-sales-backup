...
import {
  buildGoogleSheetImportRows,
  cleanGoogleSheetValue,
  parseGoogleSheetCsv,
  parseGoogleSpreadsheetSource,
} from "../../../../lib/google-sheet-import";
...
function spreadsheetSource(value: unknown) {
  const text = clean(value);
  const matched = text.match(
    /^https:\/\/docs\.google\.com\/spreadsheets\/d\/([a-zA-Z0-9_-]+)(?:\/.*)?$/,
  );
  if (!matched) {
    throw new Error("구글 스프레드시트 공유 링크를 입력해 주세요.");
  }
  ...
  const gid = /^\d+$/.test(rawGid) ? rawGid : "";
  return { spreadsheetId: matched[1], gid };
}
...
function findHeaderRow(rows: string[][]) {
  ...
  for (let rowIndex = 0; rowIndex < Math.min(rows.length, 100); rowIndex += 1) {
    ...
    const find = (names: Set<string>) =>
      normalized.findIndex((header) => names.has(header));
...
function buildImportRows(
  ...
  const importRows = [...grouped.entries()].map(([groupKey, groupRows], index) => {
    const organization = preferFullInstitutionName(
      ...groupRows.map((row) => row.organization),
    );
    const region =
      groupRows
        .map((row) => row.region)
    ...
    if (groupRows.length > 1) {
      warnings.push(`원본 ${groupRows.length}행을 기관 1건으로 묶었습니다.`);
    }
    if (rawNames.length > 1) {
      warnings.push(`기관명 표기 ${rawNames.length}개를 통합했습니다.`);
    }
    if (missingDates > 0) {
      warnings.push(`날짜 미확인 이력 ${missingDates}건이 포함되어 있습니다.`);
    }
...
export const googleSheetImportTestHelpers = {
  parseCsv,
  buildImportRows,
};
...
export async function POST(request: Request) {
  try {
    ...
    const source = parseGoogleSpreadsheetSource(body.url);
    const exportUrl = new URL(
      `https://docs.google.com/spreadsheets/d/${source.spreadsheetId}/export`,
    );
    exportUrl.searchParams.set("format", "csv");
    if (source.gid) exportUrl.searchParams.set("gid", source.gid);
    ...
    if (!response.ok) {
      return Response.json(
        {
          error:
            "구글 시트를 읽지 못했습니다. 링크가 있는 모든 사용자가 볼 수 있도록 공유했는지 확인해 주세요.",
        },
        { status: 422 },
      );
    }
    ...
    if (!csv || new TextEncoder().encode(csv).byteLength > MAX_CSV_BYTES) {
      return Response.json(
        { error: "시트가 너무 큽니다. 한 탭의 크기를 6MB 이하로 줄여 주세요." },
        { status: 413 },
      );
    }
    if (/^\s*<!doctype html/i.test(csv) || /^\s*<html/i.test(csv)) {
      return Response.json(
        {
          error:
            "구글 로그인 화면이 반환되었습니다. 링크 공유 범위를 다시 확인해 주세요.",
        },
        { status: 422 },
      );
    }
    const rows = parseGoogleSheetCsv(csv.replace(/^\uFEFF/, ""));
    const analyzed = buildGoogleSheetImportRows(rows, source);
    if (!analyzed.importRows.length) {
      return Response.json(
        { error: "등록할 기관을 찾지 못했습니다." },
        { status: 422 },
      );
    }
    const sheetTitle =
      cleanGoogleSheetValue(
        rows[0]?.find((cell) => cleanGoogleSheetValue(cell)),
      ) || "구글 시트";
    return Response.json({
      sheetTitle,
      spreadsheetId: source.spreadsheetId,
      gid: source.gid,
      rows: analyzed.importRows,
      stats: {
        headerRow: analyzed.headerRow,
        sourceRowCount: analyzed.sourceRows.length,
        institutionCount: analyzed.importRows.length,
        duplicateRowCount: analyzed.duplicateRowCount,
        missingDateCount: analyzed.missingDateCount,
        invalidDateCount: analyzed.invalidDateCount,
        missingRegionCount: analyzed.missingRegionCount,
        missingOrganizationCount: analyzed.missingOrganizationCount,
        inheritedOrganizationCount: analyzed.inheritedOrganizationCount,
      },
    });
  } catch (error) {
    ...
    if (error instanceof DOMException && error.name === "TimeoutError") {
      return Response.json(
        { error: "구글 시트를 불러오는 시간이 길어졌습니다. 다시 시도해 주세요." },
        { status: 504 },
      );
    }
    if (error instanceof Error && /구글|제목|기관/.test(error.message)) {
