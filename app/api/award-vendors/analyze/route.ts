...
function outputText(payload: any) { return payload.output_text || (payload.output ?? []).flatMap((o: any) => o.content ?? []).map((c: any) => c.text ?? "").join(""); }
...
export async function POST(request: Request) {
  try {
    ...
    const stored = await getAwardVendorBucket().get(document.object_key);
