...
async function primaryRuntimeEnabled() {
  if (await isVercelPrimaryMode()) return true;
  const role = serverValue("SITE_OPERATING_MODE").toLowerCase();
...
export async function POST(request: Request) {
  ...
  if (!automaticSyncEnabled()) {
    const schedule = (await primaryRuntimeEnabled())
  ...
  if (await primaryRuntimeEnabled()) {
    return Response.json(
      {
        ok: false,
        cutoverComplete: true,
        error: "Sites is already the primary system; replica overwrite is blocked",
      },
      { status: 409 },
    );
  }
    ...
    return Response.json(
      {
        ok: false,
        cutoverComplete: true,
        error: "Sites is already the primary system; scheduling is blocked",
      },
      { status: 409 },
    );
  }
