...
export async function GET(request: Request) {
  try {
    ...
    const awardSummaryScope = searchParams.get("scope") === "award-summary";
    const salesSummaryScope = searchParams.get("scope") === "sales-summary";
    ...
    const [primaryD1] = await Promise.all([
      ensureRecordsReady(),
      ensureJointProjectsReady(),
    ]);
    const d1 = dashboardScope || awardSummaryScope || salesSummaryScope ? getReadD1() : primaryD1;
    if (awardSummaryScope) {
    ...
    if (dashboardScope) {
      const dashboardResult = await d1
        .prepare(
          `WITH limited_activities AS (
             SELECT *
             FROM activities
             ORDER BY
               CASE WHEN follow_up_required = 1 THEN 0 ELSE 1 END,
           ...
           joint_member_candidates AS (
             ...
             FROM limited_activities source_activity
      ...
      const dashboardRecords = (
        dashboardResult.results as Record<string, unknown>[]
      )
        .slice(0, limit)
        .map((record) =>
      ...
      const dashboardHasMore = dashboardResult.results.length > limit;
      return Response.json({
        ...
        pagination: {
          limit,
          offset,
          returned: dashboardRecords.length,
          hasMore: dashboardHasMore,
          nextOffset: dashboardHasMore
            ? offset + dashboardRecords.length
            : null,
        },
      });
    }
