...
export async function POST(request: Request) {
  ...
  try {
    ...
    const [storedPdf, storedXlsx] = await Promise.all([
      storeFile({ file: new File([pdf], pdfName, { type: pdf.type || "application/pdf" }), contextType: "authored-quotation-pdf" }),
      storeFile({ file: new File([xlsx], xlsxName, { type: xlsx.type || "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" }), contextType: "authored-quotation-xlsx" }),
    ]);
    ...
    const finalize = await d1.prepare(`UPDATE authored_quotations
      ...
      .run();
    if (Number(finalize.meta.changes) !== 1) throw new Error("더 최신 견적 저장 작업이 시작되어 이전 파일 반영을 중단했습니다.");
    uploadedFileIds.length = 0;
    if (!sourceFile && existingSourceId && desiredSourceName) await organizeDriveFile(existingSourceId, folderSegments, desiredSourceName);
