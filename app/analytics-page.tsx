      ...
      <article className="panel analytics-execution-card">
        <header>
          <div>
            ...
            <h3>직영·컨소 수주 구성</h3>
            <p>{selectedYear}년 납품 완료 수주를 협업 유형과 월별 구성으로 비교합니다.</p>
          </div>
          <div className="analytics-execution-controls">
            <button type="button" onClick={() => void toggleTvMode()}>
              {tvMode ? "전체화면 종료" : "전체화면"}
            </button>
            <div className="analytics-execution-filter" aria-label="협업 유형 필터">
              {([['all', '전체'], ['direct', '직영'], ['consortium', '컨소']] as const).map(([value, label]) => <button type="button" key={value} className={trendFilter === value ? "active" : ""} onClick={() => setTrendFilter(value)}>{label}</button>)}
            </div>
            <select
              aria-label="비교 지표"
              value={trendMetric}
            ...
            >
              ...
              <option value="margin">예상수익</option>
            </select>
          </div>
        </header>
        ...
        <div className="analytics-execution-overview">
          <div className="analytics-execution-ratio compact">
            <div
              ...
              aria-label={`연간 합계 ${formatTrendValue(executionTrend.totals.total, trendMetric)}, 직영 ${(executionTrend.directRatio * 100).toFixed(1)}%, 컨소 ${(executionTrend.consortiumRatio * 100).toFixed(1)}%`}
              style={{
                background: `conic-gradient(#4f67e8 0 ${(executionTrend.directRatio * 100).toFixed(2)}%, #f39a62 ${(executionTrend.directRatio * 100).toFixed(2)}% 100%)`,
              }}
            >
              <span>
                <small>연간 합계</small>
            ...
            <div className="analytics-execution-ratio-legend">
              <button type="button" className={trendFilter === "direct" ? "active" : ""} onClick={() => setTrendFilter((value) => value === "direct" ? "all" : "direct")}>
                <i className="direct" />
              ...
              <button type="button" className={trendFilter === "consortium" ? "active" : ""} onClick={() => setTrendFilter((value) => value === "consortium" ? "all" : "consortium")}>
                <i className="consortium" />
          ...
          <section className="analytics-execution-bar-panel primary">
            <header><div><strong>월별 직영·컨소 구성</strong><small>같은 달의 직영과 컨소 실적을 크게 나란히 비교합니다.</small></div><div><span><i className="direct" />직영</span><span><i className="consortium" />컨소</span></div></header>
            <div className="analytics-execution-chart-wrap">
              <div className="analytics-execution-chart grouped" aria-label="월별 직영·컨소 비교 막대 그래프">
              {executionTrend.months.map((month) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="analytics-execution-bars">
                      {trendFilter !== "consortium" && <i className="direct" style={{ height: `${directHeight}%` }} />}
                      {trendFilter !== "direct" && <i className="consortium" style={{ height: `${consortiumHeight}%` }} />}
                    </span>
