      ...
      <section className="site-layout-guide" aria-label="현장 실측 단계">
        ...
        <div className="site-layout-guide-card">
          ...
          {workflowMode === "guided" && activeStageItems.length > 0 && <section className="site-layout-guided-stage-items" aria-label={`${activeStep.label} 등록 객체`} style={{ display: "grid", gap: 8, marginTop: 12, padding: 12, border: "1px solid #d8e0f0", borderRadius: 12, background: "#fff" }}>
            <div className="site-layout-guided-stage-head"><div><b>{activeStep.label} 등록 객체 {activeStageItems.length}개</b><small>객체를 선택해 치수를 확인하거나 수정할 수 있습니다.</small></div><details className="site-layout-stage-manage"><summary>목록 관리</summary><button type="button" className="danger" onClick={removeAllItems}>모든 객체 삭제</button></details></div>
            <ul className="site-layout-guided-stage-list">{activeStageItems.map((stageItem) => <li key={stageItem.id} className={selectedId === stageItem.id ? "active" : ""}>
              ...
              <button type="button" className="site-layout-object-edit" onClick={() => editGuidedItem(stageItem)}>수정</button><details className="site-layout-object-more"><summary aria-label={`${stageItem.name} 추가 작업`}>더보기</summary><div><button type="button" onClick={() => duplicateGuidedItem(stageItem)}>복사</button><button type="button" className="danger" onClick={() => removeItemById(stageItem.id)}>삭제</button></div></details>
            </li>)}</ul>
          </section>}
